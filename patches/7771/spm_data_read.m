...
if isfield(V(1),'is_synthetic') && V(1).is_synthetic, cl = 'synth'; end
...
switch cl
    ...
    case 'synth'
        if isempty(varargin)
            V = spm_synth_vol_read(V);
            
            if numel(V) == 1
                Y = V.data;
            else
                n = prod(V(1).dim);
                Y = zeros([V(1).dim numel(V)]);

                for i=1:numel(V)
                    Y(((i - 1) * n + 1):(i * n)) = V(i).data(:);
                end
            end
        elseif ischar(varargin{1}) && ~isequal(varargin{1},':')
            switch lower(varargin{1})
                case 'slice'
                    error('Slicing is not yet implemented for synthetic volumes.'); 
                case 'xyz'
                    Y = spm_get_data(V,varargin{2});
                otherwise
                    error('Unknown input option.');
